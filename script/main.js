...
const subjects = [
  ...
  { id: "M2.054", name: "Treball de fi de màster", credits: 15 },
  /* MISTIC */
...
function generarMenuNavegacio() {
  ...
  const links = [
    ...
    { href: "index.2012-13.html", label: "2012/13" },
  ...
  if (navList.children.length > 0 && navList.querySelector("a.current")) return;
  navList.innerHTML = "";
  links.forEach(link => {
...
// Accessors segurs per a les variables definides a cada pàgina
function getAnysRessaltats() {
  if (typeof window.anysRessaltats !== 'undefined') return window.anysRessaltats;
  if (typeof anysRessaltats !== 'undefined') return anysRessaltats;
  return [];
}

function getPreuMatricula() {
  if (typeof window.preuMatricula !== 'undefined') return window.preuMatricula;
  if (typeof preuMatricula !== 'undefined') return preuMatricula;
  return {};
}

function getPieChartConfig() {
  if (typeof window.pieChartConfig !== 'undefined') return window.pieChartConfig;
  if (typeof pieChartConfig !== 'undefined') return pieChartConfig;
  return { title: "" };
}

// Carrega el contingut HTML de tots els elements amb include-html, incloent els recursius
async function loadIncludeHTML() {
  let elements = document.querySelectorAll('[include-html]');
  let iterations = 0;

  // Permet fins a 10 nivells d'include-html recursiu
  while (elements.length > 0 && iterations < 10) {
    iterations++;
    ...
    await Promise.all(
      Array.from(elements).map(async (el) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(`Error ${response.status}: no s'ha pogut carregar ${file}`);
          }

          const html = await response.text();

          // Inserim el contingut dins l'element
          el.innerHTML = html;

          // Eliminem l'atribut per evitar recàrregues
          el.removeAttribute('include-html');

        } catch (error) {
          console.error(`Error carregant el contingut de ${file}:`, error);
          el.removeAttribute('include-html');
        }
      })
    );

    // Tornem a buscar nous includes que hagin aparegut dins el contingut carregat
    elements = document.querySelectorAll('[include-html]');
  }
}

// Càlcul de crèdits de cada màster/curs a partir de les assignatures presents al DOM
function calcularCredits() {
...
// Mostrar els crèdits ECTS aprovats + anivelladors al final del títol de cada màster
function actualitzarTitolsCredits() {
  courses.forEach(course => {
    const container = document.getElementById(course.label);
    if (container) {
      const existingSpan = container.querySelector('.credits-info');
      if (existingSpan) existingSpan.remove();

      const credits = document.createElement("span");
      credits.className = "credits-info";
      credits.textContent = `${course.title} (${course.aprovats + course.anivelladors} ECTS)`;
...
function plotGraficaCredits() {
  ...
  const labels = ['10/11-1', '10/11-2', '11/12-1', '11/12-2',
    '12/13-1', '12/13-2', '13/14-1', '13/14-2',
    '23/24-1', '23/24-2', '24/25-1', '24/25-2',
    '25/26-1', '25/26-2', '26/27-1', '26/27-2',
    '27/28-1', '27/28-2', '28/29-1', '28/29-2'
  ];

  const data = [15, 20, 32, 22, 7.5, 7.5, 11.5, 7.5, 18, 34, 24, 24, 30, 34, 34, 34, 27, 30, 36, 36];
  const mean = data.reduce((sum, value) => sum + value, 0) / data.length;

  // Convertir anys a índexs
  const ress = getAnysRessaltats();
  const idxs = ress.map(any => labels.indexOf(any));
...
function plotGraficaPercentatge() {
  ...
  if (pieChartCanvas) {
    ...
    if (pieLabels.length > 0 && pieValues.length > 0) {
      config.labels = pieLabels;
      config.data = pieValues;
    }
...
function inicialitzarDashboard() {
  ...
  courses.forEach(course => {
    ...
    const preuCurs = preus[course.label] || 0;
    tdPreu.textContent = new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR', minimumFractionDigits: 2, maximumFractionDigits: 2, useGrouping: true }).format(preuCurs);
...
// ----------------------------------------------
function inicialitzarCursosExpandibles() {
  const courseElements = document.querySelectorAll('.course');
  courseElements.forEach(course => {
    if (course.querySelector('.course-expand-btn')) return;
...
// Funció principal d'orquestració
async function init() {
  await loadIncludeHTML();
  generarMenuNavegacio();
  calcularCredits();
  actualitzarTitolsCredits();
  actualitzarCopyright();
  inicialitzarDashboard();
  inicialitzarCursosExpandibles();
}

// Exposar funcions al context global per a màxima flexibilitat
window.loadIncludeHTML = loadIncludeHTML;
window.includeHTML = loadIncludeHTML;
window.initApp = init;
